...
function transcript(client, msglimit) {
    
  //log that it got loaded on bot start
  console.log(`Module Transcript started`)
  ...
  let temporarymsg = message.channel.send(new MessageEmbed().setAuthor("Transcripting...", "https://cdn.discordapp.com/emojis/757632044632375386.gif?v=1"))
  let docx = officegen({
    type: 'docx',
  ...
  message.react("📑"); //react to the message as a prove that everything above worked!
  //The "TITLE" 
  ...
  let channelMessages = message.channel.messages.fetch({//fetch the last 100 messages
    limit: 100
  ...
  while (channelMessages.size === 100) { //make a loop if there are more then 100 messages in this channel to fetch
    ...
    channelMessages = message.channel.messages.fetch({ limit: 100, before: lastMessageId }).catch(err => console.log(err)); //Fetch again, 100 messages above the already fetched messages
